...
int read_thread(void* arg){
    ...
    read_hash_file(args->opt.file, args->opt.dir, args->in_q);
    q_final(args->in_q);
    return 0;
}
...
void check(struct options opt) {
    ...
    thrd_t reader;
    thrd_t writers[opt.num_threads];

    thrd_create(&reader,read_thread,&a);
    for(int i = 0; i < opt.num_threads; i++) thrd_create(&writers[i],writer_thread,&a);


    for(int i = 0; i < opt.num_threads; i++) thrd_join(writers[i],NULL);
    // Liberar memoria de reader
