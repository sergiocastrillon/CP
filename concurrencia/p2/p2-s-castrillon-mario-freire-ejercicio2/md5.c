...
void add_files(char *entry, void *arg) {
    ...
    if(S_ISREG(st.st_mode)){
        if(q_insert(a->q, strdup(entry))== -1){
...
void sum(struct options opt) {
    ...
    int dirname_len;
    in_q  = q_create(opt.queue_size);
    out_q = q_create(opt.queue_size+2000);
    ///////////////////////////////////////////
    thrd_t reader;
    struct args args;
...
// Transpaso a archivo de salida
    ...
    //thrd_join(reader,NULL); // No deberia de llegar el hilo hasta aqui
